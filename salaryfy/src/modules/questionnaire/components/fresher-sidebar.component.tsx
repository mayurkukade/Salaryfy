...
export default function SideBar() {
  ...
  const [pageActive, setPageActive] = React.useState({ fresherDashboard: false, profileUpload: false, roadMap: false });

  function updateLocation() {
    const newPagesActive = { ...pageActive };

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    Object.entries(newPagesActive).map(([key, value]: [string, boolean]) => { newPagesActive[key as keyof typeof newPagesActive] = false })
    switch (location.pathname) {
      case SIDE_TAB_PAGES.FRESHER_DASHBOARD:
        newPagesActive.fresherDashboard = true; break;
      case SIDE_TAB_PAGES.FRESHER_PROFILE_UPLOAD:
        newPagesActive.profileUpload = true; break;
      case SIDE_TAB_PAGES.FRESHER_ROADMAP:
        newPagesActive.roadMap = true; break;
    }
  ...
  function IconHolder({ children, active, onClick }: { children: React.ReactNode, active: boolean, onClick: () => void }) {
    return (<div onClick={onClick} className={`cursor-pointer h-[6em] w-[7em] flex items-center justify-center bg-[${active ? '#005F59' : 'white'}]`}>{React.Children.map(children, child => React.cloneElement(child as ReactElement<Record<string, boolean>, string | JSXElementConstructor<Record<string, boolean>>>, { active }))}</div>);
  }
  function IconMobileHolder({ children, active, onClick }: { children: React.ReactNode, active: boolean, onClick: () => void }){
    return(
      <div className='cursor-pointer h-[6em] w-[7em] flex justify-center items-center bg-[white]' onClick={onClick}>{React.Children.map(children, child => React.cloneElement(child as ReactElement<Record<string, boolean>, string | JSXElementConstructor<Record<string, boolean>>>, { active }))}</div>
    );
  }
  ...
  function PageIcon({ active }: { active?: boolean }) {
    return (
      <svg width="22" height="26" viewBox="0 0 22 26" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path stroke={active ? 'white' : '#005F59'} d="M13 1H3.4C2.76348 1 2.15303 1.25286 1.70294 1.70294C1.25286 2.15303 1 2.76348 1 3.4V22.6C1 23.2365 1.25286 23.847 1.70294 24.2971C2.15303 24.7471 2.76348 25 3.4 25H17.8C18.4365 25 19.047 24.7471 19.4971 24.2971C19.9471 23.847 20.2 23.2365 20.2 22.6V8.2M13 1L20.2 8.2M13 1V8.2L20.2 8.2M15.3987 14.1999H5.79869M15.3987 19H5.79869M8.19869 9.39989H5.79869" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  function LinesIcon({ active }: { active?: boolean }) {
    return (
      <svg width="19" height="24" viewBox="0 0 19 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M17 22V9.5" stroke={active ? 'white' : '#005F59'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M9.5 22V2" stroke={active ? 'white' : '#005F59'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M2 22V14.5" stroke={active ? 'white' : '#005F59'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  function PageMobileIcon({ active }: {active?: boolean}) {
    return (
      <svg width="42" height="47" viewBox="0 0 42 47" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path stroke={ active ? '#005F59' : '#B3B3B3' } d="M23.2004 11.7061L14.0057 11.7061C13.3961 11.7061 12.8114 11.9482 12.3803 12.3793C11.9492 12.8104 11.707 13.3951 11.707 14.0047L11.707 32.3941C11.707 33.0038 11.9492 33.5885 12.3803 34.0195C12.8114 34.4506 13.3961 34.6928 14.0057 34.6928H27.7978C28.4074 34.6928 28.9921 34.4506 29.4232 34.0195C29.8542 33.5885 30.0964 33.0038 30.0964 32.3941L30.0964 18.6021M23.2004 11.7061L30.0964 18.6021M23.2004 11.7061V18.6021L30.0964 18.6021M25.4978 24.3487H16.3031M25.4978 28.9461L16.3031 28.9461M18.6018 19.7513H16.3031" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
  ...
  function LinesMobileIcon({ active }: {active?: boolean}) {
    return (
      <svg width="18" height="23" viewBox="0 0 18 23" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M16 21.3635L16 9.26123" stroke={ active ? '#005F59' : '#B3B3B3' } strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M9 21.3636L9 2" stroke={ active ? '#005F59' : '#B3B3B3' } strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M2 21.3634L2 14.1021" stroke={ active ? '#005F59' : '#B3B3B3' } strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  return (
    <>
      <div className="top-[0] pt-[10em] fixed h-[100%] w-[100%] z-50 flex items-end lg:items-start pointer-events-none">
        <div className='gap-[2em] flex-col bg-[white] w-[max-content] hidden lg:flex h-[max-content] pointer-events-auto'>
          <IconHolder onClick={() => onTabIconClick(SIDE_TAB_PAGES.FRESHER_DASHBOARD)} active={pageActive.fresherDashboard}><HomeIcon /></IconHolder>
          <IconHolder onClick={() => onTabIconClick(SIDE_TAB_PAGES.FRESHER_PROFILE_UPLOAD)} active={pageActive.profileUpload}><PageIcon /></IconHolder>
          <IconHolder onClick={() => onTabIconClick(SIDE_TAB_PAGES.FRESHER_ROADMAP)} active={pageActive.roadMap}><LinesIcon /></IconHolder>
        </div>
        <div className="flex lg:hidden flex-row bg-[white] flex-grow justify-around py-[1em] pointer-events-auto" style={{ boxShadow: '0px -8px 10px -5px rgba(0, 0, 0, 0.3)' }}>
          <IconMobileHolder onClick={() => onTabIconClick(SIDE_TAB_PAGES.FRESHER_DASHBOARD)} active={pageActive.fresherDashboard}><HomeMobileIcon /></IconMobileHolder>
          <IconMobileHolder onClick={() => onTabIconClick(SIDE_TAB_PAGES.FRESHER_PROFILE_UPLOAD)} active={pageActive.profileUpload}><PageMobileIcon /></IconMobileHolder>
          <IconMobileHolder onClick={() => onTabIconClick(SIDE_TAB_PAGES.FRESHER_ROADMAP)} active={pageActive.roadMap}><LinesMobileIcon /></IconMobileHolder>
        </div>
      </div>
      <Outlet />
    </>
