...
import { useGetAllJobFairDetailsQuery } from "../../../features/api-integration/screeningQuestion/screeningQuestionStep2Slice";
...
export default function ScreeningQuestions() {
  ...
  const {data,error, isLoading} = useGetAllJobFairDetailsQuery(currentPage);
  console.log(`${data} is from backend` )
  console.log(`Current Page value is ${currentPage}`);
//Function to handle back page 
  ...
  return (
    <div className='w-100 flex flex-col items-center h-[100%]'>
      ...
      <div className='flex items-center bg-[#FECD08] px-[1.5em] py-[0.5em] rounded-xl mx-[1em]' >
        <button className='text-[2em] font-medium mr-[0.5em] text-[#005F59] cursor-pointer' onClick={nextPageHandler}>Next</button>
