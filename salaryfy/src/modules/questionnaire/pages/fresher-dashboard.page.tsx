...
import {
  EducationalSkillsType,
  FresherProfileUploadService,
  INITIAL_EDUCATIONAL_SKILLS,
} from "../services/fresher-profile-upload.service";
import {
  EducationalSkillsKey,
  UserEducationSkill,
} from "../constants/fresher.type";
...
import {
  ...
  STREAM_LIST,
  STREAM_NOBOARD_LIST,
} from "../constants/fresher-profile-upload.list";
import {
  useLazyGetProfileQuery,
  useLazyUniversitySuggestionsQuery,
  useSaveProfileMutation,
} from "../../../features/api-integration/profile-qualification/profile-qualification.slice";
...
export function FresherDashboard() {
  ...
  const userProfile = useSelector(
    (state: AppStoreStateType) => state.root[SLICE_NAMES.USER_DETAILS]
  );
  ...
  const [educationalSkills, setEducationalSkills] =
    useState<EducationalSkillsType>(INITIAL_EDUCATIONAL_SKILLS);
  ...
  const [listUpcomingInterviews, setListUpcomingInterviews] = useState<
    Array<UpcomingInterviewType>
  >([]);
  const [recommendJobsState, setRecommendJobsState] = useState<
    Array<JobsDetailsType>
  >([]);
  ...
  useEffect(() => {
    if (educationalSkillsChangedByRef.current.value === CHANGED_BY.USER) {
      const selectedHighestEducation =
        educationalSkills.highestEducationList.find((e) => e.selected);
      switch (selectedHighestEducation?.value) {
        case HIGHEST_EDUCATION.MATRIC:
          ...
          setEducationalSkills((educationalSkills) => ({
            ...educationalSkills,
            boardList: BOARD_LIST.map((board) => ({
              ...board,
              selected: false,
            })),
            streamList: [],
        ...
        case HIGHEST_EDUCATION.INTER:
          ...
          setEducationalSkills((educationalSkills) => ({
            ...
            streamList: STREAM_LIST.map((stream) => ({
              ...stream,
              selected: false,
            })),
          }));
        ...
        default:
          setEducationalSkills((educationalSkills) => ({
            ...educationalSkills,
            boardList: [],
            streamList: STREAM_NOBOARD_LIST.map((stream) => ({
              ...stream,
              selected: false,
            })),
          }));
  ...
  useEffect(() => {
    const payload = {
      highestEducation: educationalSkills.highestEducationList.find(
        (entity) => entity.selected
      )?.value,
      board: educationalSkills.boardList.find((entity) => entity.selected)
        ?.value,
      stream: educationalSkills.streamList.find((entity) => entity.selected)
        ?.value,
    ...
    educationalSkills.highestEducationList.find((entity) => entity.selected)
      ?.value,
    educationalSkills.boardList.find((entity) => entity.selected)?.value,
    educationalSkills.streamList.find((entity) => entity.selected)?.value,
    educationalSkills.percentage,
  ]);
  ...
  async function userIdUpdated() {
    ...
    } = await getLazyUserProfile(userId.toString());
    ...
    if (userProfile) {
      ...
      httpClient
        .request(getLazyUpcomingInterviews(userId))
        .pipe(concatMap(async ({ data: { list: response } }) => response))
        .subscribe((response) => {
          const listUpcomingInterviewsResponse =
            response as Array<UpcomingInterviewType>;
  ...
  function fetchEducationalSkills(userId: string) {
    httpClient
      .request(getProfileEducationalSkills(userId))
      ...
      .subscribe((response: UserEducationSkill) => {
        ...
        if (response?.highestLevelOfEdu) {
          onHighestLevelEducationChangeHandler(
            response.highestLevelOfEdu,
            CHANGED_BY.SERVER
          );

          educationalSkillsKey.highestLevelEducation[1](
            () => response.highestLevelOfEdu || null
          );
          payloadEducationalSkills.highestLevelEducation[1](
            () => response.highestLevelOfEdu || null
          );
        }
        if (response?.board) {
  ...
  function onHighestLevelEducationChangeHandler(
    value: string | null,
    ...
    if (value) {
      ...
      setEducationalSkills((educationalSkills) =>
        fresherProfileUploadService.onHighestLevelEducationChange(
          educationalSkills,
          value
        )
      );
  ...
  function onBoardUniversityFieldInput(value: string) {
    const selectedHighestEducation =
      educationalSkills.highestEducationList.find((e) => e.selected);
    if (
      selectedHighestEducation?.value !== HIGHEST_EDUCATION.MATRIC &&
      selectedHighestEducation?.value !== HIGHEST_EDUCATION.INTER
    ) {
      httpClient
        .request(getUniversities(value))
        .pipe(
          concatMap(
            async ({ data: { list: response } }) =>
              response?.map(
                (entity: Record<string, string>) => entity.board_University
              ) || []
          )
        )
        .subscribe((universitiesList: string[]) =>
          setEducationalSkills((educationalSkills) => ({
            ...educationalSkills,
            boardList: universitiesList.map((university: string) => ({
              code: university,
              value: university,
              selected: false,
            })),
  ...
  function onBoardUniversityChangeHandler(value: string | null) {
    if (value) {
      ...
      setEducationalSkills((educationalSkills) =>
        fresherProfileUploadService.onBoardListChange(educationalSkills, value)
      );
    }
  }
  ...
  function onStreamChangeHandler(value: string | null) {
    if (value) {
      ...
      setEducationalSkills((educationalSkills) =>
        fresherProfileUploadService.onStreamListChange(educationalSkills, value)
      );
    }
  }

  function onPercentageChangeHandler({
    target: { value },
  }: ChangeEvent<HTMLInputElement>) {
    if (!/^[0-9]{0,3}$/gi.test(value)) {
      return;
    }
  ...
  async function onSaveFresherInfo() {
    const payload: UserEducationSkill = {
      highestLevelOfEdu:
        educationalSkillsKey.highestLevelEducation[0] || undefined,
      board: educationalSkillsKey.board[0] || undefined,
      stream: educationalSkillsKey.stream[0] || undefined,
      percentage: _.inRange(educationalSkills.percentage, 0, 101)
        ? educationalSkills.percentage
        : undefined,
      UserId: userId,
    };

    if (payload.highestLevelOfEdu === HIGHEST_EDUCATION.MATRIC) {
      payload.stream = null;
    } else {
      ...
      <div className=" flex flex-col gap-5 md:flex-row ">
        ...
        <div className="flex flex-col">
          ...
          <div
            ...
            className="mb-[2em] xxs:text-[0.8rem] md:text-[1.8em] rounded-xl overflow-hidden grid grid-cols-[max-content,auto] grid-cols-auto [&>*]:whitespace-nowrap [&>*]:px-[1.5em] [&>*]:py-[0.5em] [&>*]:border-b [&>*]:border-solid [&>*]:border-b-[#0E5F591A] [&>*:nth-child(odd)]:bg-[#0E5F5919] [&>*:nth-child(odd)]:text-[#5b5b5b] [&>*:nth-child(odd)]:w-[10em] [&>*:nth-child(even)]:w-[100%] [&>*:nth-child(even)]:break-all"
          >
            <div>Full Name</div>
            ...
            <div>
              {true && (
                <span className="bg-[yellow] rounded-[1em] text-[green] px-[0.5em] py-[0.25em]">
                  In Progress
                </span>
              )}
            </div>
            ...
            <div>
              {userProfile?.date &&
                CommonUtilities.date.formatDate(userProfile?.date)}
            </div>
          ...
          <div
            ...
            className="rounded-xl lg:mb-[2em] xs:mb-[2em] p-3"
          >
            ...
            <EducationalInfoComponent
              ...
              educationalSkillsKey={educationalSkillsKey}
              onHighestLevelEducationChangeHandler={
                onHighestLevelEducationChangeHandler
              }
              onBoardUniversityFieldInput={onBoardUniversityFieldInput}
              onBoardUniversityChangeHandler={onBoardUniversityChangeHandler}
              onStreamChangeHandler={onStreamChangeHandler}
              onPercentageChangeHandler={onPercentageChangeHandler}
            />
            <Button
              style={{ minWidth: "10em", lineHeight: "27px", fontSize: "18px" }}
              size="large"
              variant="contained"
              onClick={onSaveFresherInfo}
            >
              Update
            </Button>
          </div>
          ...
          <div className="text-[2.6em] font-[500] my-[1em]">
            Recommended jobs
          </div>
          ...
          <div className="flex flex-col gap-6">
            {
              ...
              recommendJobsState.map((details: JobsDetailsType) => (
                <JobCard2
                  key={CommonUtilities.generateRandomString(100)}
                  details={details}
                />
              ))
            }
        ...
        <div className="flex flex-col pr-[1em] md:pl-[5em] pt-[1.5rem]">
          ...
          <div className="p-4 app-scrollbar h-[50em] flex flex-col gap-[2em]">
            {listUpcomingInterviews.map(
              (upcomingInterview: UpcomingInterviewType) => (
                <UpcomingInterviewCard
                  details={upcomingInterview}
                  key={CommonUtilities.generateRandomString(20)}
                />
              )
            )}
          </div>
...
function UpcomingInterviewCard({
  ...
  useEffect(() => {
    async function init() {
      const {
        data: { object: jobDetailsResponse },
      } = await getLazyJobById(details.jobId.toString());
      ...
      <div className="flex flex-col max-w-[32em]">
        <div className="flex mb-[1em]">
          ...
          <div className="text-[1.5em] font-semibold">
            <div className="text-[#0E5F59]">{jobDetails?.postName}</div>
